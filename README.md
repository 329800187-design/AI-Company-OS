...
当前已推进到 Phase 8.7：Phase 6 通用流程与 Boss 执行闭环、Phase 7C 测试基线、Phase 7D 本地一键部署验证，以及 Phase 8.1–8.7 的经营记忆与人工动作治理均已在工作区完成。当前动作连接器仍为 `local_simulation`，真实外部系统连接和跨平台干净环境验收尚未完成，详见 [当前项目状态](docs/current_project_state.md)。
...
最后保存时间：2026-07-13（最近更新 2026-07-28：Phase 6 全部完成、Phase 7C 测试基线清零、进入 Phase 7D 部署运行手册与一键启动验证）
...
- 🩺 **部署体检** — 本地服务全量体检脚本
- ✅ **一键部署验收** — 环境预检、前端生产构建、临时后端、核心 API 与交付中心 smoke 闭环
...
详细部署、Docker、失败排查和发布清单见 `docs/phase7d_deployment_runbook.md`。
