...
## 分类 B：minidelivery task listing（18 个失败）

**文件：** `tests/test_minidelivery.py`（全部 18 个 TestListTasks 失败）

**根因：** `_create_task()` helper 在 `tmp_path` 创建测试数据，但 `GET /minidelivery/tasks` 返回空列表。
...
**可能原因：**
1. `_create_task()` helper 创建的文件格式与当前 minidelivery router 期望的格式不匹配
2. `OUTPUT_ROOT` patch 路径不正确
3. minidelivery router 的 task listing 逻辑变更（Phase 6 通用化影响）

**建议处理：**
- 优先检查 `_create_task()` helper 是否与当前 minidelivery 数据格式一致
- 如果是格式变更导致，更新 helper 和测试断言
