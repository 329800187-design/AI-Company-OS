...
本架构描述组织边界和协作方式，不为核心系统预设特定行业、平台或交付物。默认 Boss 模板、模块、提示词、可见文案和报告必须保持通用业务流程表达：具体行业、渠道和案例只能作为用户输入、示例、兼容别名或可选插件出现。

所有跨部门任务遵循“生成计划 -> 人工确认 -> 执行一次 -> 人工验收”的边界；任何外部动作必须在人工批准后执行。历史文档中出现的具体业务案例不构成默认产品方向。当前实施状态和兼容边界以 `docs/current_project_state.md` 为准。
