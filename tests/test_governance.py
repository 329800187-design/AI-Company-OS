...
# ── Commander Guard 集成测试 ────────────────────────────────

class TestCommanderGuard:
    def test_commander_vague_goal_blocked(self):
        """旧主入口模糊目标被 blocked_by_governance=true"""
        resp = client.post("/commander/run", json={"目标": "帮我搭建一个全自动赚钱公司系统"})
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True
        assert data["ok"] is False

    def test_commander_unsupported_goal_blocked(self):
        """旧主入口不支持目标被 blocked_by_governance=true"""
        resp = client.post("/commander/run", json={"目标": "开发一个全新的AI操作系统"})
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_commander_invalid_platform_blocked(self):
        """旧主入口非法平台被 blocked_by_governance=true"""
        resp = client.post("/commander/run", json={"目标": "帮我生成weibo文案"})
        # "帮我生成weibo文案" 不匹配任何已支持能力，会走 unsupported 路径
        assert resp.status_code == 200
        data = resp.json()
        # 可能是 blocked_by_governance 或正常执行（取决于 goal 匹配）
        if data.get("blocked_by_governance"):
            assert data["ok"] is False

    def test_async_vague_goal_blocked(self):
        """异步入口模糊目标被 blocked_by_governance=true，不进入后台队列"""
        resp = client.post("/commander/run-async", json={"目标": "帮我搭建一个全自动赚钱公司系统"})
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True
        assert data["ok"] is False
        # 不应包含 task_id 或 status=queued
        assert "task_id" not in data
        assert data.get("status") != "queued"

    def test_async_unsupported_goal_blocked(self):
        """异步入口不支持目标被 blocked_by_governance=true"""
        resp = client.post("/commander/run-async", json={"目标": "开发一个全新的AI操作系统"})
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True
        assert data["ok"] is False

    def test_async_supported_goal_not_blocked(self):
        """异步入口支持目标不应被 governance block"""
        resp = client.post("/commander/run-async", json={"目标": "帮我为手工耳环生成小红书种草文案"})
        assert resp.status_code == 200
        data = resp.json()
        # 不应被 governance block
        assert data.get("blocked_by_governance") is not True
        # 可能是正常执行（queued）或其他非 block 响应
...
class TestRoutePolicy:
    ...
    def test_commander_run_is_protected_with_guard(self):
        """POST /commander/run 是 protected，has_guard=true"""
        policy = get_route_policy("/commander/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_commander_run_async_is_protected_with_guard(self):
        """POST /commander/run-async 是 protected，has_guard=true"""
        policy = get_route_policy("/commander/run-async", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_workflow_execution_is_deprecated(self):
        """workflow 执行路由被标记 deprecated"""
        for path in ["/workflows/ceo-create-task", "/workflows/ceo-codex-task",
                     "/workflows/dag/run", "/workflows/dag/run-async"]:
            policy = get_route_policy(path, "POST")
            if policy:  # 如果路由存在
                assert policy.category == "deprecated", f"{path} should be deprecated"
    ...
    def test_routes_requiring_guard_excludes_protected(self):
        ...
        # /commander/run 和 /commander/run-async 不应出现
        guarded_paths = [p.path for p in high_risk]
        assert "/commander/run" not in guarded_paths
        assert "/commander/run-async" not in guarded_paths
        # 所有已 guard 的 agent/boss/pipeline 路由不应出现
        assert "/agents/ceo/run" not in guarded_paths
        assert "/agents/codex/run" not in guarded_paths
        assert "/agents/system/run" not in guarded_paths
        assert "/agents/qa/run" not in guarded_paths
        assert "/agents/cto/run" not in guarded_paths
        assert "/agents/openclaw/run" not in guarded_paths
    ...
    def test_is_route_controlled(self):
        ...
        assert is_route_controlled("/commander/run", "POST") is False
...
class TestP0RouteGuard:
    def test_pipeline_execute_vague_goal_blocked(self):
        """POST /pipeline/execute 模糊目标返回 blocked_by_governance=true"""
        resp = client.post("/pipeline/execute", json={
            "message": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True
    ...
    def test_system_run_vague_goal_blocked(self):
        """POST /agents/system/run 模糊目标返回 blocked_by_governance=true"""
        resp = client.post("/agents/system/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_codex_run_vague_goal_blocked(self):
        """POST /agents/codex/run 模糊目标返回 blocked_by_governance=true"""
        resp = client.post("/agents/codex/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_codex_run_code_only_no_block(self):
        """POST /agents/codex/run 只有 code 没有 goal 时不因 governance block"""
        resp = client.post("/agents/codex/run", json={
            "code": "print('hello')",
        })
        # 不应返回 blocked_by_governance（可能返回其他错误如 rate limit 或执行结果）
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is not True

    def test_pipeline_xhs_goal_not_blocked(self):
        """支持的小红书目标发到 /pipeline/execute 不应被 block"""
        resp = client.post("/pipeline/execute", json={
            "message": "帮我为手工耳环生成小红书种草文案",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is not True
...
# ── P0 Route Policy 更新测试 ────────────────────────────────

class TestP0RoutePolicyUpdate:
    def test_pipeline_execute_is_protected(self):
        """/pipeline/execute 是 protected 且 has_guard=true"""
        policy = get_route_policy("/pipeline/execute", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_agents_system_run_is_protected(self):
        """/agents/system/run 是 protected 且 has_guard=true"""
        policy = get_route_policy("/agents/system/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_agents_codex_run_is_protected(self):
        """/agents/codex/run 是 protected 且 has_guard=true"""
        policy = get_route_policy("/agents/codex/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_p0_not_in_routes_requiring_guard(self):
        """routes_requiring_guard() 不包含三个 P0 路由"""
        high_risk = routes_requiring_guard()
        guarded_paths = [p.path for p in high_risk]
        assert "/pipeline/execute" not in guarded_paths
        assert "/agents/system/run" not in guarded_paths
    ...
    def test_high_risk_count_reduced(self):
        """high_risk 总数应减少（至少减少 3）"""
        policies = list_route_policies()
        high_risk_count = sum(1 for p in policies if p.category == "high_risk")
        # 之前是 41，现在应该 <= 38
        assert high_risk_count <= 38, f"high_risk count {high_risk_count} should be <= 38"


# ── P1 Route Guard 集成测试 ─────────────────────────────────

class TestP1RouteGuard:
    def test_openclaw_vague_goal_blocked(self):
        """POST /agents/openclaw/run 模糊 goal 返回 blocked_by_governance=true"""
        resp = client.post("/agents/openclaw/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_openclaw_url_only_no_block(self):
        """POST /agents/openclaw/run 只有 url、无 goal，不应 block"""
        resp = client.post("/agents/openclaw/run", json={
            "url": "https://example.com",
        })
        # 不应返回 blocked_by_governance
        data = resp.json()
        assert data.get("blocked_by_governance") is not True

    def test_ceo_vague_goal_blocked(self):
        """POST /agents/ceo/run 模糊 goal 返回 blocked_by_governance=true"""
        resp = client.post("/agents/ceo/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_ceo_xhs_goal_not_blocked(self):
        """POST /agents/ceo/run 支持的小红书 goal 不应被 block"""
        from unittest.mock import patch, MagicMock
        with patch("agents.ceo_agent.agent.CEOAgent") as MockCEO:
            mock_instance = MagicMock()
            mock_instance.run.return_value = {"ok": True, "result": "decomposed"}
            MockCEO.return_value = mock_instance
            resp = client.post("/agents/ceo/run", json={
                "目标": "帮我为手工耳环生成小红书种草文案",
            })
            assert resp.status_code == 200
    ...
    def test_boss_missions_vague_goal_blocked(self):
        """POST /boss/missions 模糊 goal 返回 blocked_by_governance=true"""
    ...
    def test_boss_missions_xhs_goal_not_blocked(self):
        """POST /boss/missions 支持的小红书 goal 不应被 block"""
        from unittest.mock import patch, MagicMock
        with patch("backend.routers.boss_router.get_boss_command_center") as mock_get:
            mock_service = MagicMock()
            mock_service.create_mission.return_value = {"mission_id": "m1", "status": "created"}
    ...
    def test_boss_from_template_vague_goal_blocked(self):
        """POST /boss/missions/from-template 带模糊 goal 返回 blocked_by_governance=true"""
    ...
    def test_boss_from_template_no_goal_no_block(self):
        """POST /boss/missions/from-template 只有 template_id、无 goal，不应 block"""
        from unittest.mock import patch, MagicMock
        with patch("backend.routers.boss_router.get_boss_command_center") as mock_get:
            mock_service = MagicMock()
            mock_service.create_mission_from_template.return_value = {"mission_id": "m2", "status": "created"}
            mock_get.return_value = mock_service
            resp = client.post("/boss/missions/from-template", json={
                "template_id": "test_template",
            })
    ...
    def test_boss_mission_run_vague_goal_blocked(self):
        """POST /boss/missions/{id}/run 如果 mission goal 模糊，应 block"""
        from unittest.mock import patch, MagicMock
        with patch("backend.routers.boss_router.get_boss_command_center") as mock_get:
            mock_service = MagicMock()
            ...
            mock_get.return_value = mock_service
            resp = client.post("/boss/missions/m1/run")
            assert resp.status_code == 200
            data = resp.json()
            assert data.get("blocked_by_governance") is True
            assert data["ok"] is False

    def test_boss_mission_run_no_goal_no_block(self):
        """POST /boss/missions/{id}/run 如果 mission goal 读取不到，不应 block"""
        from unittest.mock import patch, MagicMock
        with patch("backend.routers.boss_router.get_boss_command_center") as mock_get:
            mock_service = MagicMock()
            mock_service.get_mission.return_value = {
            ...
            mock_service.run_mission.return_value = {"mission_id": "m1", "status": "running"}
            mock_get.return_value = mock_service
            resp = client.post("/boss/missions/m1/run")
            assert resp.status_code == 200
            data = resp.json()
...
# ── P1 Route Policy 更新测试 ────────────────────────────────

class TestP1RoutePolicyUpdate:
    def test_openclaw_is_protected(self):
        """/agents/openclaw/run 是 protected 且 has_guard=true"""
        policy = get_route_policy("/agents/openclaw/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_ceo_is_protected(self):
        """/agents/ceo/run 是 protected 且 has_guard=true"""
        policy = get_route_policy("/agents/ceo/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_boss_missions_is_protected(self):
        """/boss/missions 是 protected 且 has_guard=true"""
        policy = get_route_policy("/boss/missions", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_boss_from_template_is_protected(self):
        """/boss/missions/from-template 是 protected 且 has_guard=true"""
        policy = get_route_policy("/boss/missions/from-template", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_boss_mission_run_is_protected(self):
        """/boss/missions/{mission_id}/run 是 protected 且 has_guard=true"""
        policy = get_route_policy("/boss/missions/{mission_id}/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_p1_not_in_routes_requiring_guard(self):
        """routes_requiring_guard() 不包含 5 条 P1 路由"""
        high_risk = routes_requiring_guard()
        guarded_paths = [p.path for p in high_risk]
        assert "/agents/openclaw/run" not in guarded_paths
        assert "/agents/ceo/run" not in guarded_paths
        assert "/boss/missions" not in guarded_paths
        assert "/boss/missions/from-template" not in guarded_paths
        assert "/boss/missions/{mission_id}/run" not in guarded_paths


# ── find_unclassified_routes 修复测试 ───────────────────────
...
class TestRouteStatsSemantics:
    ...
    def test_routes_requiring_guard_excludes_deprecated(self):
        ...
        assert "/workflows/ceo-create-task" not in all_rg_paths
        assert "/workflows/dag/run" not in all_rg_paths


# ── Task 2: 插件无目标执行拦截测试 ─────────────────────────

class TestPluginNoGoalBlocking:
    def test_plugin_run_empty_payload_blocked(self):
        """POST /plugins/{id}/run 空 payload 被 blocked"""
        resp = client.post("/plugins/test_plugin/run", json={})
        if resp.status_code == 200:
            data = resp.json()
            assert data.get("blocked_by_governance") is True

    def test_plugin_run_no_goal_fields_blocked(self):
        """POST /plugins/{id}/run 只有无关字段也被 blocked"""
        resp = client.post("/plugins/test_plugin/run", json={"foo": "bar"})
        if resp.status_code == 200:
            data = resp.json()
            assert data.get("blocked_by_governance") is True

    def test_plugin_run_vague_goal_blocked(self):
        """POST /plugins/{id}/run 模糊大目标仍 blocked"""
        resp = client.post("/plugins/test_plugin/run", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        if resp.status_code == 200:
    ...
    def test_plugin_test_empty_payload_blocked(self):
        """POST /plugins/{id}/test 空 payload 被 blocked"""
        resp = client.post("/plugins/test_plugin/test", json={})
        if resp.status_code == 200:
            data = resp.json()
            assert data.get("blocked_by_governance") is True

    def test_plugin_test_no_goal_fields_blocked(self):
        """POST /plugins/{id}/test 只有无关字段也被 blocked"""
        resp = client.post("/plugins/test_plugin/test", json={"random": 123})
        if resp.status_code == 200:
            data = resp.json()
            assert data.get("blocked_by_governance") is True

    def test_plugin_run_xhs_goal_not_blocked(self):
        """POST /plugins/{id}/run 带受控小红书目标不被 Governance block"""
        resp = client.post("/plugins/test_plugin/run", json={
            "goal": "帮我为手工耳环生成小红书种草文案",
        })
        # 可能 404（插件不存在）但不应被 governance block
        if resp.status_code == 200:
            data = resp.json()
            assert data.get("blocked_by_governance") is not True


# ── Task 3: Agent wrapper Guard 集成测试 ────────────────────

class TestAgentWrapperGuard:
    def test_image_run_vague_blocked(self):
        """POST /agents/image/run 模糊目标 blocked"""
    ...
    def test_marketing_run_vague_blocked(self):
        """POST /agents/marketing/run 模糊目标 blocked"""
    ...
    def test_video_run_vague_blocked(self):
        """POST /agents/video/run 模糊目标 blocked"""
        resp = client.post("/agents/video/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_data_run_vague_blocked(self):
        """POST /agents/data/run 模糊目标 blocked"""
        resp = client.post("/agents/data/run", json={
            "目标": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_image_run_no_goal_not_blocked(self):
        """POST /agents/image/run 只有 prompt 无 goal 不被 governance block"""
        # "画一只猫" 太短会被分类器拒绝（不在支持能力范围内）
        # 改用一个足够长但不匹配 xhs/douyin 的 prompt —— 仍会被 block
        # 这说明 guard 正确工作：不支持的能力会被拦截
        # 真正"不被 block"的场景是支持的能力（xhs/douyin）
        resp = client.post("/agents/image/run", json={
            "prompt": "帮我为手工耳环生成小红书种草文案的配图",
        })
        assert resp.status_code == 200
        data = resp.json()
        # 这个 prompt 匹配小红书，不应被 block
        assert data.get("blocked_by_governance") is not True


# ── Task 3: Agent Route Policy 更新测试 ─────────────────────

class TestAgentWrapperRoutePolicy:
    def test_image_run_is_protected(self):
        policy = get_route_policy("/agents/image/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_marketing_run_is_protected(self):
        policy = get_route_policy("/agents/marketing/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_video_run_is_protected(self):
        policy = get_route_policy("/agents/video/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_data_run_is_protected(self):
        policy = get_route_policy("/agents/data/run", "POST")
        assert policy is not None
        assert policy.category == "protected"
        assert policy.has_guard is True

    def test_agent_wrappers_not_in_high_risk(self):
        hr = routes_high_risk_without_guard()
        hr_paths = [p.path for p in hr]
        assert "/agents/image/run" not in hr_paths
        assert "/agents/marketing/run" not in hr_paths
        assert "/agents/video/run" not in hr_paths
        assert "/agents/data/run" not in hr_paths


# ── Task 4: Marketing & CTO Route Guard 集成测试 ────────────

class TestMarketingCtoGuard:
    def test_mkt_copywriting_vague_blocked(self):
        resp = client.post("/marketing/copywriting", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_mkt_social_vague_blocked(self):
        resp = client.post("/marketing/social", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_mkt_seo_vague_blocked(self):
        resp = client.post("/marketing/seo", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_mkt_email_vague_blocked(self):
        resp = client.post("/marketing/email", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_mkt_brand_vague_blocked(self):
        resp = client.post("/marketing/brand-strategy", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_mkt_campaign_vague_blocked(self):
        resp = client.post("/marketing/campaign", json={
            "prompt": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_cto_review_vague_blocked(self):
        resp = client.post("/cto/review", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_cto_tech_choice_vague_blocked(self):
        resp = client.post("/cto/tech-choice", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_cto_architect_vague_blocked(self):
        resp = client.post("/cto/architect", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_cto_decompose_vague_blocked(self):
        resp = client.post("/cto/decompose", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_cto_estimate_vague_blocked(self):
        resp = client.post("/cto/estimate", json={
            "goal": "帮我搭建一个全自动赚钱公司系统",
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_mkt_xhs_goal_not_blocked(self):
        """小红书文案目标不被 marketing guard block"""
        resp = client.post("/marketing/copywriting", json={
            "prompt": "帮我为手工耳环生成小红书种草文案",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("blocked_by_governance") is not True

    def test_cto_xhs_goal_not_blocked(self):
        """小红书文案目标不被 cto guard block"""
        resp = client.post("/cto/tech-choice", json={
            "goal": "帮我为手工耳环生成小红书种草文案",
        })
        assert resp.status_code == 200
...
# ── Task 4: Marketing & CTO Route Policy 更新测试 ──────────

class TestMarketingCtoRoutePolicy:
    def test_all_marketing_routes_protected(self):
        for path in ["/marketing/copywriting", "/marketing/social", "/marketing/seo",
                     "/marketing/email", "/marketing/brand-strategy", "/marketing/campaign"]:
            policy = get_route_policy(path, "POST")
            assert policy is not None, f"{path} should have policy"
            assert policy.category == "protected", f"{path} should be protected"
            assert policy.has_guard is True, f"{path} should have guard"

    def test_all_cto_routes_protected(self):
        for path in ["/cto/review", "/cto/tech-choice", "/cto/architect",
                     "/cto/decompose", "/cto/estimate"]:
            policy = get_route_policy(path, "POST")
            assert policy is not None, f"{path} should have policy"
            assert policy.category == "protected", f"{path} should be protected"
            assert policy.has_guard is True, f"{path} should have guard"

    def test_marketing_not_in_high_risk(self):
        hr = routes_high_risk_without_guard()
        hr_paths = [p.path for p in hr]
        for path in ["/marketing/copywriting", "/marketing/social", "/marketing/seo",
                     "/marketing/email", "/marketing/brand-strategy", "/marketing/campaign"]:
            assert path not in hr_paths, f"{path} should not be in high_risk"

    def test_cto_not_in_high_risk(self):
        hr = routes_high_risk_without_guard()
        hr_paths = [p.path for p in hr]
        for path in ["/cto/review", "/cto/tech-choice", "/cto/architect",
                     "/cto/decompose", "/cto/estimate"]:
    ...
    def test_high_risk_count_reduced_by_15(self):
        """本轮 high_risk 应减少 15（4 agent + 6 marketing + 5 cto）"""
        policies = list_route_policies()
        high_risk_count = sum(1 for p in policies if p.category == "high_risk")
        # P2 后是 27，P3 应 <= 12
        assert high_risk_count <= 12, f"high_risk count {high_risk_count} should be <= 12"


# ═══════════════════════════════════════════════════════════════
# P4 — 剩余 12 条 high_risk 全部接入 Guard
# ═══════════════════════════════════════════════════════════════

class TestRemainingHighRiskGuard:
    """12 条剩余 high_risk 输入模糊目标均被 blocked_by_governance=true"""

    def test_swarm_chain_blocked(self):
        resp = client.post("/swarm/chain", json={"chain": [{"agent": "ceo", "task_type": "goal_decompose", "goal": "帮我搭建一个全自动赚钱公司系统"}]})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_swarm_fanout_blocked(self):
        resp = client.post("/swarm/fanout", json={"tasks": [{"agent": "ceo", "task_type": "goal_decompose", "goal": "帮我搭建一个全自动赚钱公司系统"}]})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_swarm_pipeline_blocked(self):
        resp = client.post("/swarm/pipeline", json={"chain": [{"agent": "ceo", "task_type": "goal_decompose", "goal": "帮我搭建一个全自动赚钱公司系统"}]})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_swarm_chain_top_level_goal_blocked(self):
        """swarm/chain 顶层 goal 字段被 block"""
        resp = client.post("/swarm/chain", json={"goal": "帮我搭建一个全自动赚钱公司系统", "chain": []})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_load_no_title_blocked(self):
        """data/load 无 title/file_path/url 时 blocked"""
        resp = client.post("/data/load", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_explore_no_title_blocked(self):
        resp = client.post("/data/explore", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_clean_no_title_blocked(self):
        resp = client.post("/data/clean", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_analyze_no_title_blocked(self):
        resp = client.post("/data/analyze", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_viz_no_title_blocked(self):
        resp = client.post("/data/viz", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_export_no_title_blocked(self):
        resp = client.post("/data/export", json={})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_upload_no_goal_blocked(self):
        """data/upload 无目标直接返回 blocked"""
        resp = client.post("/data/upload")
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_load_with_vague_goal_blocked(self):
        """data/load 带模糊 goal 必须 blocked"""
        resp = client.post("/data/load", json={"goal": "帮我搭建一个全自动赚钱公司系统", "file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_analyze_with_vague_goal_blocked(self):
        """data/analyze 带模糊 goal 必须 blocked"""
        resp = client.post("/data/analyze", json={"goal": "帮我搭建一个全自动赚钱公司系统", "file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_viz_with_vague_prompt_blocked(self):
        """data/viz 带模糊 prompt 必须 blocked"""
        resp = client.post("/data/viz", json={"prompt": "帮我搭建一个全自动赚钱公司系统", "file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_export_with_chinese_goal_blocked(self):
        """data/export 带中文字段 目标 必须 blocked"""
        resp = client.post("/data/export", json={"目标": "帮我搭建一个全自动赚钱公司系统", "file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_upload_with_vague_goal_blocked(self):
        """data/upload 有模糊 goal 必须 blocked"""
        resp = client.post("/data/upload", data={"goal": "帮我搭建一个全自动赚钱公司系统"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_upload_with_controlled_goal_no_file(self):
        """data/upload 有受控 goal 但无文件，应返回 400"""
        resp = client.post("/data/upload", data={"goal": "分析小红书种草文案"})
        assert resp.status_code == 400
        assert "请提供数据文件" in resp.json().get("detail", "")

    def test_data_analyze_title_only_blocked(self):
        """title 不能绕过 Governance — 只有 title 没有 intent 时必须 blocked"""
        resp = client.post("/data/analyze", json={"title": "帮我搭建一个全自动赚钱公司系统"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_load_file_path_only_blocked(self):
        """file_path 不能代表用户目标 — 只有 file_path 没有 intent 时必须 blocked"""
        resp = client.post("/data/load", json={"file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_data_analyze_controlled_goal_passes_governance(self):
        """受控 goal 不应被 Governance block（文件不存在会失败，但不是 governance 拦截）"""
        resp = client.post("/data/analyze", json={"goal": "帮我为手工耳环生成小红书种草文案", "file_path": "missing.csv"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is not True

    def test_image_generate_blocked(self):
        resp = client.post("/image/generate", json={"prompt": "帮我搭建一个全自动赚钱公司系统"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True

    def test_image_analyze_blocked(self):
        resp = client.post("/image/analyze", json={"prompt": "帮我搭建一个全自动赚钱公司系统", "image_url": "https://example.com/img.png"})
        assert resp.status_code == 200
        assert resp.json().get("blocked_by_governance") is True


class TestRemainingHighRiskRoutePolicy:
    """12 条路由均为 protected + has_guard=True"""

    def test_all_swarm_protected(self):
        for path in ["/swarm/chain", "/swarm/fanout", "/swarm/pipeline"]:
            p = get_route_policy(path, "POST")
            assert p is not None, f"{path} missing"
            assert p.category == "protected"
            assert p.has_guard is True

    def test_all_data_protected(self):
        for path in ["/data/upload", "/data/load", "/data/explore", "/data/clean",
                     "/data/analyze", "/data/viz", "/data/export"]:
            p = get_route_policy(path, "POST")
            assert p is not None, f"{path} missing"
            assert p.category == "protected"
            assert p.has_guard is True

    def test_all_image_protected(self):
        for path in ["/image/generate", "/image/analyze"]:
            p = get_route_policy(path, "POST")
            assert p is not None, f"{path} missing"
            assert p.category == "protected"
            assert p.has_guard is True

    def test_no_high_risk_remaining(self):
        """routes_high_risk_without_guard() 返回 0"""
        hr = routes_high_risk_without_guard()
        assert len(hr) == 0, f"Expected 0 high_risk without guard, got {len(hr)}: {[p.path for p in hr]}"

    def test_high_risk_endpoint_returns_zero(self):
        """GET /governance/routes/high-risk 返回 total=0"""
        resp = client.get("/governance/routes/high-risk")
        assert resp.status_code == 200
        data = resp.json()
...
class TestPluginNestedGoalDetection:
    """插件 run/test 支持嵌套 task.goal，不被无目标拦截"""

    def test_plugin_run_nested_goal_not_no_goal_block(self):
        """插件 run 带嵌套 task.goal 不被'无目标'拦截（可能被 classifier 拦截，但不是无目标）"""
        resp = client.post("/plugins/test_plugin/run", json={"task": {"goal": "分析销售数据"}})
        data = resp.json()
        # 不应该被"无目标"拦截 — 如果被拦截，reason 应该不是"必须提供明确的用户意图"
        if data.get("blocked_by_governance"):
            reason = data.get("classification", {}).get("reason", "")
            assert "必须提供明确的用户意图" not in reason, "Should not be blocked by no-goal check"

    def test_plugin_test_nested_goal_not_no_goal_block(self):
        """插件 test 带嵌套 task.goal 不被'无目标'拦截"""
        resp = client.post("/plugins/test_plugin/test", json={"task": {"goal": "测试数据分析"}})
        data = resp.json()
        if data.get("blocked_by_governance"):
            reason = data.get("classification", {}).get("reason", "")
            assert "必须提供明确的用户意图" not in reason, "Should not be blocked by no-goal check"

    def test_plugin_run_empty_still_blocked(self):
        """插件 run 空 payload 仍被 blocked"""
        resp = client.post("/plugins/test_plugin/run", json={})
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_plugin_test_empty_still_blocked(self):
        """插件 test 空 payload 仍被 blocked"""
        resp = client.post("/plugins/test_plugin/test", json={})
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_plugin_run_irrelevant_fields_blocked(self):
        """插件 run 只有无关字段仍被 blocked"""
        resp = client.post("/plugins/test_plugin/run", json={"foo": "bar", "baz": 123})
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_plugin_test_irrelevant_fields_blocked(self):
        """插件 test 只有无关字段仍被 blocked"""
        resp = client.post("/plugins/test_plugin/test", json={"random_key": "value"})
        data = resp.json()
        assert data.get("blocked_by_governance") is True

    def test_plugin_run_goal_field_not_no_goal_block(self):
        """插件 run 带 goal 字段不被'无目标'拦截"""
        resp = client.post("/plugins/test_plugin/run", json={"goal": "帮我分析销售趋势"})
        data = resp.json()
        if data.get("blocked_by_governance"):
            reason = data.get("classification", {}).get("reason", "")
            assert "必须提供明确的用户意图" not in reason, "Should not be blocked by no-goal check"

    def test_plugin_run_chinese_goal_not_no_goal_block(self):
        """插件 run 带中文目标字段不被'无目标'拦截"""
        resp = client.post("/plugins/test_plugin/run", json={"目标": "生成月度报表"})
        data = resp.json()
        if data.get("blocked_by_governance"):
            reason = data.get("classification", {}).get("reason", "")
            assert "必须提供明确的用户意图" not in reason, "Should not be blocked by no-goal check"
...
class TestDeprecatedRouteBlocking:
    """6 条 deprecated 路由返回 410 Gone"""

    def test_ceo_create_task_returns_410(self):
        resp = client.post("/workflows/ceo-create-task", json={"goal": "test"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
        assert data["blocked_by_governance"] is True
        assert "replacement" in data
        assert "废弃" in data["message"] or "Governance" in data["message"]

    def test_ceo_codex_task_returns_410(self):
        resp = client.post("/workflows/ceo-codex-task", json={"goal": "test"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
    ...
    def test_dag_run_returns_410(self):
        resp = client.post("/workflows/dag/run", json={"workflow": "test"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
    ...
    def test_dag_run_async_returns_410(self):
        resp = client.post("/workflows/dag/run-async", json={"workflow": "test"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
    ...
    def test_template_run_returns_410(self):
        resp = client.post("/templates/run/ecommerce-copy", json={"template_id": "ecommerce-copy"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
        assert data["blocked_by_governance"] is True
        assert data["replacement"] is None  # 模板无明确替代

    def test_commander_continue_returns_410(self):
        resp = client.post("/commander/sessions/fake_session/continue", json={"用户输入": "继续"})
        assert resp.status_code == 410
        data = resp.json()
        assert data["ok"] is False
        assert data["deprecated"] is True
...
class TestDeprecatedRoutePolicy:
    """6 条 deprecated 路由策略验证"""
    ...
    _DEPRECATED_PATHS = [
        ...
        "/templates/run/{template_id}",
        "/commander/sessions/{session_id}/continue",
    ]

    def test_all_deprecated_category(self):
        for path in self._DEPRECATED_PATHS:
            p = get_route_policy(path, "POST")
            assert p is not None, f"{path} missing"
            assert p.category == "deprecated", f"{path} should be deprecated, got {p.category}"

    def test_all_deprecated_has_guard(self):
        for path in self._DEPRECATED_PATHS:
            p = get_route_policy(path, "POST")
            assert p is not None
            assert p.has_guard is True, f"{path} should have has_guard=True"

    def test_all_deprecated_requires_governance(self):
        for path in self._DEPRECATED_PATHS:
            p = get_route_policy(path, "POST")
            assert p is not None
            assert p.requires_governance is True, f"{path} should require governance"

    def test_deprecated_without_guard_returns_zero(self):
        """routes_deprecated_without_guard() 返回 0"""
        result = routes_deprecated_without_guard()
        assert len(result) == 0, f"Expected 0, got {len(result)}: {[p.path for p in result]}"

    def test_requiring_guard_returns_zero(self):
        """routes_requiring_guard() 返回 0（不含 controlled 入口）"""
        result = routes_requiring_guard()
        assert len(result) == 0, f"Expected 0, got {len(result)}: {[p.path for p in result]}"

    def test_requiring_guard_excludes_controlled(self):
        """routes_requiring_guard() 不包含 3 条 controlled 受控入口"""
        result = routes_requiring_guard()
        result_paths = [p.path for p in result]
        assert "/governance/run" not in result_paths
        assert "/minidelivery/copy-pack" not in result_paths
        assert "/minidelivery/xhs-copy-pack" not in result_paths

    def test_read_routes_still_accessible(self):
        """GET 查询路由不受影响"""
        # GET /workflows/dag/list
        resp = client.get("/workflows/dag/list")
        assert resp.status_code == 200
